 ...
 *
 *   // network
 *   CIQcashNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.iq.cash/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CIQcashNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
// // can be under: IQcashNetwork
// //   -- signature requirements : Key1(User)
// class CIQcashNetworkVariable : public CGovernanceObject
// {
// private:

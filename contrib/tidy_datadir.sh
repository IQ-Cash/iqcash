...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete IQcash database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no IQcash datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old IQcash datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected IQcash 0.7 datadir."
    ;;
  3)
    echo "Detected IQcash pre-0.8 datadir."
    ;;
  4)
    echo "Detected IQcash 0.8 datadir."
    ;;
esac
